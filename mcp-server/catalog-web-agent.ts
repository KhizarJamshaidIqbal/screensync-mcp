  ...
  return [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        required: ["action"],
        properties: {
          action: { type: "string", enum: ["start", "get", "stop", "viewer"], description: "Recording lifecycle or viewer rendering." },
          ...
          download: { type: "boolean", default: false, description: "On stop or viewer, save the trace (.json or .html) to Downloads." },
          filename: { type: "string", description: "Download filename for the trace." },
          format: { type: "string", enum: ["json", "html"], default: "json", description: "Output format on stop (json trace or self-contained offline html viewer)." },
          emitHtml: { type: "boolean", description: "Generate and return self-contained offline HTML viewer string on stop." },
          trace: { type: "object", description: "Existing trace object to render when action is viewer." },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        required: ["action"],
        properties: {
          action: { type: "string", enum: ["start", "stop", "status", "codegen"], description: "Recorder lifecycle or code generation." },
          name: { type: "string", description: "Flow name or test name for codegen." },
          target: { type: "string", enum: ["flow", "playwright"], default: "flow", description: "Codegen target format." },
          codegen: { type: "boolean", description: "Generate flow/playwright code on stop." },
          saveFlow: { type: "boolean", description: "Directly save the generated flow to saved flows." },
          steps: { type: "array", items: { type: "object" }, description: "Explicit steps array for action: 'codegen'." },
        },
        additionalProperties: false,
      },
    },
