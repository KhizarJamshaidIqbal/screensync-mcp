...
import { cognitiveExtendedToolDefinitions } from "./catalog-cognitive-extended.js";
...
export function cognitiveToolDefinitions(): Tool[] {
  return [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          pruneObsolete: {
            type: "boolean",
            description: "Accepted for compatibility. Obsolete playbooks (confidence <= 0.1) are ARCHIVED as deprecated, never deleted, and are never offered by web_recall."
          },
          reflect: {
            type: "boolean",
            description: "Also let the hub reflect on what has happened since the last pass and record what it worked out (per-intent reliability, failure clusters, slow steps, recurring pitfalls). Deterministic, no model call; it runs only for domains with enough new activity unless force is set. The insights come back in `reflection` and are returned by web_recall."
          },
          domain: { type: "string", description: "With reflect: reflect on just this domain (default: every domain with episodes)." },
          force: { type: "boolean", description: "With reflect: reflect even when there is not yet enough new activity." }
        }
      }
    },
    {
    ...
    ...cognitiveExtendedToolDefinitions(),
  ];
}
