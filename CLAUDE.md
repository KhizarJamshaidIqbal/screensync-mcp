...
## 7. Changelog - every build gets an entry
...
```bash
python tools/build_changelog.py            # rewrite changelog.html + changelog.json
python tools/build_changelog.py --check    # exit 1 if stale - use this as a gate
```

Then include `website/changelog.html` and `website/changelog.json` in the same commit as
the version bump, and mirror `main:website` onto the `deploy` branch so the site updates.
