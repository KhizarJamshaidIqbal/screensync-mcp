// ScreenSync CDP capture executors — full-page screenshot, PDF, MHTML, a11y tree, hook-based HAR
// export, coverage, screencast. The element/clip screenshot (web_element_screenshot) lives in
// web-element-screenshot.js: split out to stay under the repo's 500-line ceiling (AGENTS.md §2).
import { rawAttach, rawDetach, attachCdp, detachCdp, main, activeScreencasts, activeCoverage, cdpWaitPaintReady } from './web-adv-core.js';
...
export async function cdpScreenshot(tab, args = {}) {
  ...
  try {
    ...
    // Same compositor-lag guard as tab-resolve.js's waitForPaintReady, shared with every CDP
    // capture path via cdpWaitPaintReady (web-adv-core.js).
