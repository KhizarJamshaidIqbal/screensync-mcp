...
import { DEFAULT_TOKEN, SSE_UNAUTHORIZED_RETRY_MS } from './constants.js';

/**
 * @param {import('./sse-client.js').SseClient} client
 * @param {{ getSettings?: () => Promise<object>, getInstanceId?: () => Promise<string>, now?: () => number }} deps
 */
export function createSseSupervisor(client, { getSettings = readSettings, getInstanceId, now = () => Date.now() } = {}) {
  async function credentials() {
    ...
    return { url: normalizeHubBase(s.hubUrl), token, instanceId };
  }

  /**
   * Returns what it did: 'stopped' | 'start' | 'restart' | 'unauthorized' | 'backoff' | 'ok'.
  ...
  async function ensure(reason = 'ensure', { retryUnauthorized = false } = {}) {
    ...
    const opts = { instanceId: cred.instanceId };
    ...
    return client.state === 'backoff' ? 'backoff' : 'ok';
  }
