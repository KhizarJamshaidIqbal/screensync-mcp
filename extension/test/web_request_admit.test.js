...
const { admitWebRequest, resetAdmittedRequests } = await import('../lib/web-request-admit.js');
...
// ── expired: less than a second of the hub's wait left ──
assert.deepEqual(admitWebRequest(req('late', { deadlineAt: NOW + 999 }), NOW), { ok: false, reason: 'expired' });
assert.deepEqual(admitWebRequest(req('gone', { deadlineAt: NOW - 60_000 }), NOW), { ok: false, reason: 'expired' });
assert.deepEqual(admitWebRequest(req('late', { deadlineAt: NOW + 5_000 }), NOW), { ok: true }, 'an expired id is not remembered');
...
// ── web-bridge: a refused request runs nothing and posts nothing ──
{
  ...
  const live = { ...req('b1'), deadlineAt: Date.now() + 30_000, targetInstanceId: 'inst_self' };
  await handleWebRequest(live);
  const results = () => posts.filter((p) => p.url.endsWith('/api/web/result'));
  assert.equal(results().length, 1, 'a fresh request is answered');
  ...
  await handleWebRequest({ ...live, id: 'b2', deadlineAt: Date.now() - 1 });
